...
class Carrier extends AppModel{
    public $title = 'Shipping Service';
    private $paginationSettings = array();

    public $virtualFields = array(
        'nameFull' => "concat(Carrier.name,' - ', Carrier.service)"
    );
    ...
    public function afterFind($results, $useless = false){
        ...
        foreach($resultsArray as &$result) {
            if(isset($result[$this->alias]['logo']) and $result[$this->alias]['logo']){
                $result[$this->alias]['logoFull'] = Configure::read('urlImg') . 'logos/carrier/' . $result[$this->alias]['logo'];
            }
            else{
    ...
    public function paginationSettings(){
        if(empty($this->paginationSettings)){
            $this->paginationSettings = array(
                'order' => $this->alias . '.name,' . $this->alias . '.service',
                'limit' => 50
            );
        }
